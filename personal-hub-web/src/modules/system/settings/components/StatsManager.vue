...
onMounted(async () => {
  ...
  if (listRef.value) {
    sortable = Sortable.create(listRef.value, {
      ...
      onEnd: (evt) => {
        const items = layoutStore.statsCards as CardItem[]
...
async function persist() {
  await layoutStore.saveStatsConfig([...layoutStore.statsCards])
}

async function handleReset() {
  await layoutStore.resetStatsConfig()
  ElMessage.success('统计布局已恢复默认')
}
</script>
...
<template>
  <div class="card-manager">
    ...
    <div ref="listRef" class="chip-row">
      <button
        v-for="card in layoutStore.statsCards"
        :key="card.code"
        type="button"
        class="card-chip"
