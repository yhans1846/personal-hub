...
应用文件日志（Logback，按天滚动，保留 30 天）挂载在宿主机：

```
/data/personal-hub/logs/personal-hub.log
/data/personal-hub/logs/personal-hub.yyyy-MM-dd.log
/data/personal-hub/logs/personal-hub-error.log
```

可通过环境变量 `LOG_PATH` 覆盖目录（默认 `/data/personal-hub/logs`）。

容器标准输出仍可用 Compose 查看：
...
# 文件日志
sudo tail -f /data/personal-hub/logs/personal-hub.log
```
