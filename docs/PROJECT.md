...
### 第四阶段（UI + 通知）
- [x] **UI 优化**: 设计令牌刷新/侧边栏分组/共享组件/Dashboard 扩展/Topbar 增强/响应式/Accent Color/Command Palette/MD 编辑器
- [x] **功能增强**: 待办拖拽/阅读评分和时长/日记地点和配图/学习计划超期标记/笔记阅读时间
- [x] **通知系统**: sys_notification 表 / 自动检测生成（TODO超期/计划截止/计划完成）/ 定时任务 / 前端 Bell 红标下拉面板
