...
可读优先 · 命名/结构统一 · 领域对齐 · 公共沉 common · 慎加依赖 · 前后端命名一致 · RESTful + Result · 表设计一致 · 改代码先改文档 · **根治问题，不拆东墙补西墙（修改涉及多链路的逻辑时，必须检查所有调用方，用统一机制覆盖全场景）**
...
| 二十二 | 笔记列表 Overlay + 三栏分屏编辑器；文件一键清空；列表工具栏按钮统一 |
| 二十三 | 笔记列表文件夹树（CRUD + 拖拽归属/层级；备份纳入） |
| 二十四 | 笔记语雀式首页壳（默认首页 · 树挂笔记 · 点夹保留列表） |
...
| 笔记三栏分屏（编辑/预览/大纲 · pane 滚动联动） | ✅ |
| 笔记列表文件夹树（直属筛选 · 拖拽） | ✅ |
| 笔记首页壳（默认首页 · 树挂笔记） | ✅ |
| 文件管理一键清空 · 列表工具栏 `.toolbar-btn` 统一 | ✅ |
...
| 43 | Overlay 写作；三栏分屏；文件清空；工具栏样式统一 |
