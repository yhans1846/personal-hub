...
- **修复**：编辑页右侧目录收起后找不到展开——`EditorOutline` 补 `side="right"`，去掉强制 `width:100%`，收起成右缘窄轨并保留顶部展开钮
- **功能**：笔记左栏升级为**知识空间**（`KnowledgeSpaceNav`）——搜索内联过滤夹/笔记、首页、最近（≤5）、收藏（空则隐藏）、我的空间树；去掉「全部」；列表 / Overlay / 预览三处共用；预览只读（无＋/拖拽/行菜单），侧栏跟 `--preview-*`
- **UI**：文件夹树「未分类」固定在用户夹**之后**（有笔记才显示）
- **UI**：文件夹树「收起侧栏」改到标题栏右侧，与目录「收起」位置统一
- **UI**：文件夹树「未分类」并入「我的空间」分组（与用户夹同列）
- **UI**：笔记预览页统一阅读主题——左栏/顶栏/正文/右目录均用 `--preview-*`（随 sepia/light/dark/follow），顶栏与两侧头对齐 56px
- **修复**：`UiTooltip` 用原生元素作根并阻断错误 `role` 透传，消除预览页 Dropdown/Tooltip 相关 Vue 警告
- **功能**：笔记编辑 Overlay / 独立预览页左侧同列表知识空间（`NoteFolderShell`）；点笔记切换（编辑脏则先确认）；点夹/首页仅更新选中；当前笔记高亮；**预览只读**；**预览页目录改右侧**
...
- **文档**：规格 `2026-07-23-knowledge-space-nav-design` / `diary-image-url-and-data-purge-design`；修订 `note-home-shell-design` / PAGE_SPEC（知识空间侧栏）
...
- **UI**：笔记文件夹树——夹/未分类整枝展开收起；未分类默认收起且空则隐藏；桌面可收起为窄条（底栏按钮）；首页目录左边距
...
- **功能**：笔记语雀式首页壳——`/notes` 默认首页；树挂笔记；点夹仍进列表
...
- **功能**：笔记列表全屏 Overlay 工作区 + 默认三栏分屏（编辑/预览等宽 · 大纲约 15% · pane 比例滚动联动 · 大纲点击双侧定位）；顶栏标题/属性抽屉；模式 split\|edit\|preview；预览栏不重复标题；栏间分割线与底色区分
- **功能**：双向链接 v1（实验开关）——`[[标题]]` / `[[标题|别名]]`；补全；预览跳转；回链 API 与面板
- **修复**：预览配图灯箱改自定义遮罩（替代 medium-zoom）；回链空态隐藏；wiki 补全改写；编辑器相对路径配图代理；写作区近全宽、移除专注模式
- **文档**：同步 PROJECT / README / PAGE_SPEC / API / STYLE_GUIDE 与上述能力
...
- **部署**：Compose 暴露 MySQL/Redis 宿主机端口（`MYSQL_PUBLISH`/`REDIS_PUBLISH`），便于本机 IDE 直连
- **部署**：前端构建改用 npmmirror 安装 pnpm，避免 corepack 直连 npmjs 超时
