...
- **体验**：分屏编辑/预览按**标题锚点**联动滚动（参考观墨 Guanmo：锚点映射 + 短时锁定，非简单比例）；右侧大纲点击同时定位预览与编辑区
...
- **修复**：笔记预览配图灯箱——弃用 medium-zoom（`isAnimating` 卡死导致关不掉），改为自定义遮罩/×/Esc 关闭；状态栏统计项改为横向单行显示
- **体验**：回链面板无数据时隐藏；wiki 补全选中改写 Markdown 避免多余括号；编辑器内相对路径配图统一代理
- **UI**：笔记编辑页取消主布局 80% 限宽，写作区近全宽（左右约 24px）；移除专注模式
- **治理**：移除 Qodana GitHub Action 与 `qodana.yaml`
