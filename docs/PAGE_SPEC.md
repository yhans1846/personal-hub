...
| 编辑 | 默认 Dialog；笔记新建/编辑为**全屏 Overlay 工作区**（真实路由 `/notes/new`、`/notes/:id/edit`，刷新不丢；入口**新开标签**；Teleport；非独立整页）；只读预览仍独立页 `/notes/:id/preview` |
...
- 上传可选分类；卡片展示分类名（与收藏夹一致）；列表仅本页上传文件；工具栏无「分类管理」链接；一屏 5×4 铺满（`size=20`）；工具栏「清空」永久删除全部文件（强确认，返回条数）
...
TOC **在右侧**，宽 220（收起 34，拖 160–400）。主题：`data-preview-theme` → `--preview-*` → `--prose-*`；**整页（左树 / 顶栏 / 正文 / 右目录）统一 `--preview-*`，勿混用应用壳 `--bg-*`**。  
...
**列表：** Header+Toolbar+Empty+Pagination · Table/Card 或例外 · 槽位/fill · Dialog+深链 · Token+骨架+深色 · 工具栏次要/危险用 `.toolbar-btn`，与主按钮并排  
**笔记：** `/notes` 默认**知识库首页**（文档数 + 欢迎 + 按夹目录，点笔记开 Overlay）；左栏**知识空间**（`KnowledgeSpaceNav`：搜索 / 首页 / 最近≤5 / 收藏有则显示无则隐藏 / 我的空间树+未分类；**无「全部」**；搜索内联过滤夹与笔记标题；非只读可 CRUD/拖拽）；点夹·未分类 → 右侧**现有卡片/表格列表**（多选/导入导出保留；导入 Markdown 可多选 `.md`，每个文件一篇笔记，归属当前选中文件夹，分类/标签/资源目录共用）；最近/收藏/树内笔记 → **新标签**打开 `/notes/:id/edit`；筛选为夹**直属**笔记；新建走 **新标签** `/notes/new`（可带 `?folderId=`；首页 → 未分类）；列表新建/编辑为**全屏 Overlay 工作区**（`List.vue` 托管 + Teleport → body，刷新保留）；**默认三栏分屏**（左 Markdown 编辑 · 中实时预览 · 右标题大纲；编辑与预览等宽，大纲约 15%，固定比例 v1 不可拖拽；**分屏时编辑/预览由两侧 pane 统一滚动并按比例联动**；大纲点击定位预览与编辑区；预览栏不重复标题；栏间分割线与底色区分；锚点规则与独立预览页一致，编辑侧剥离 IR marker 后对齐 id）；顶栏可编辑**标题** +「属性」**抽屉**（分类/标签等 meta，Esc 可关抽屉）；模式 `split`（默认）\| `edit` \| `preview`（顶栏切换；Ctrl+Shift+P 在 split↔preview；≤768 默认 edit 全宽）；仅顶栏 ✕ 关闭，**Esc 不关工作区**；脏关闭须确认；路由 `/notes/new`、`/notes/:id/edit`（旧 `?create=1` / `?edit=` 自动迁移）；只读预览仍 `/notes/:id/preview`；右键「归档」→`AUTO_ARCHIVE`；「移入回收站」→`USER_DELETE`；列表多选 →「导出 ZIP」（≤50，仅勾选）；回收站展示删除原因 +「清空回收站」；编辑右键可插入 Mermaid 围栏块与 KaTeX（行内 `$` / 块级 `$$`），IR/预览/文件 md 预览常开渲染；实验开关「双向链接」开启后支持 `[[标题]]` 补全、预览跳转与回链列表；预览配图点开为自定义灯箱（点遮罩/×/Esc 关闭）；**工作区近全宽**（绕过 `--content-max-width`，无专注模式）；**编辑 Overlay / 独立预览页左侧同列表知识空间**（`NoteFolderShell`；点夹/首页仅改选中；最近/收藏/树内笔记可打开——编辑脏则先确认；当前笔记高亮；≤768 抽屉；**预览只读**——可用搜索/首页/最近/收藏/展开跳转，无＋/拖拽/行菜单；侧栏跟 `--preview-*`）  
**命令面板：** Ctrl/Cmd+K 支持快捷新建（笔记/日记/阅读/学习/待办/计划）与页面/内容搜索  
**设置·外观：** 主题/强调色/圆角/动画/密度/内容宽；**界面字体**（默认思源黑体；可选思源宋体/霞鹜文楷/Inter/系统；仅 UI，阅读/Markdown 正文仍走「阅读」）  
**设置·数据：** 独立 Tab「数据管理」——缓存清理；立即备份（落盘历史+下载）；自动备份频率；历史下载/恢复/删除；导入 `.zip` 全量覆盖；**清空数据**（字符图验证码 → 先备份再清业务，保留账号/资料/`user_layout`，历史只留该备份并下载）  
**日记：** 配图支持本地上传与「从链接添加」（服务端下载 http(s) 图片）  
**设置·高级：** 通知偏好 · 实验功能（AI「即将推出」· 双向链接可开）  
**预览：** DocLayout+prose · readingConfig · §3 注意项  
**Dialog：** 积木骨架 · FooterActions · 日记配图先保存  
**文件：** 内嵌预览 · 鉴权下载 · md 预览与笔记共用 Vditor 配置（含 Mermaid/KaTeX）· 不与笔记/日记资源混库 · 工具栏可一键清空全部文件
