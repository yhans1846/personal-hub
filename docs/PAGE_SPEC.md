...
**笔记：** 右键「归档」→`AUTO_ARCHIVE`；「移入回收站」→`USER_DELETE`；列表多选 →「导出 ZIP」（≤50，仅勾选）；回收站展示删除原因 +「清空回收站」；编辑右键可插入 Mermaid 围栏块与 KaTeX（行内 `$` / 块级 `$$`），IR/预览/文件 md 预览常开渲染；实验开关「双向链接」开启后支持 `[[标题]]` 补全、预览跳转与回链列表；预览配图点开为自定义灯箱（点遮罩/×/Esc 关闭）  
