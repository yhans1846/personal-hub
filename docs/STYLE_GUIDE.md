...
- `themeStore`：外观（主题/强调色/圆角/密度/内容宽/**界面字体 uiFont**）；`layoutStore`：菜单/工作台/统计卡片
...
- 全站 UI 字体：`--font-sans`（默认思源黑体 / Noto Sans SC）；外观可切换预置字体；**阅读/Markdown 正文字体不跟此联动**（见阅读设置）
...
界面字体：`--font-sans`（`appearanceConfig.uiFont`）；等宽：`--font-mono`。  
