...
| `DialogSection` / `DialogDivider` / `DialogFooterActions` | 业务弹窗积木（笔记除外） |
| `ImageLightbox` | 居中图片预览 |
| `CommandPalette` · `NotificationBell` · `ProfileDrawer` · `AppLayout` | 全局壳层 |
| `NoteVditor` · `NoteMarkdownPreview` · `DocLayout` | 笔记/文档 |

业务弹窗约定：壳用 `UiDialog`（`size` sm=520 / md=720 / lg=860）；分区用 `DialogSection`/`DialogDivider`；Footer 用 `DialogFooterActions`（禁止裸 `el-button`）。笔记 Full Page / 导入除外。
