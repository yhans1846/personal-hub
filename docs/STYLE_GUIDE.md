...
目录：`modules/{knowledge,planning,resource,system,dashboard,search,stats,category}` · `components/(ui/)` · `composables/` · `styles/`
...
| `UiDialog` / `UiInput` / `UiTextarea` / `UiSelect` / `UiDatePicker` / `UiButton` / `UiSection` / `UiCard` | 基础 UI |
| `CommandPalette` · `NotificationBell` · `ConfirmDialog` · `SearchBar` | 全局交互 |
| `NoteVditor` · `NoteMarkdownPreview` · `DocLayout` · `MarkdownEditor` | 笔记/文档 |
| `Tag` · `PriorityTag` · `ProgressCard` · `FileUploader` · `StatCard` | 展示 |

| composable / util | 用途 |
|-------------------|------|
| `useDeepLinkDialog` · `useMainContentFill` · `useFillPageSize` · `useProductViewMode` | 列表页（细则 `PAGE_SPEC`） |
| `readingTime` · `deepLink` | 相对时间 / 深链 |

样式：`global.css`（Token）· `product-list.css` · `markdown-prose.css`。
