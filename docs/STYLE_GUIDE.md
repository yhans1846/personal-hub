...
断点：&lt;768 侧栏 overlay；&lt;1024 Topbar 字隐。动画：路由 fade 跟随 `--transition` · 侧栏同 · 骨架 1.5s。空状态用 `EmptyState`，禁纯文字。  
...
| `useEntityDialog` · `useEntityFormSave` | Dialog 加载实体；统一 validate→create/update→提示 |
