...
public interface DashboardMapper {
    ...
    /** 笔记分类统计 */
    List<Map<String, Object>> selectCategoryStats(@Param("userId") Long userId, @Param("limit") int limit);

    /** 标签使用排行 */
    List<Map<String, Object>> selectTagStats(@Param("userId") Long userId, @Param("limit") int limit);

    /** 最近活动（审计日志） */
    List<Map<String, Object>> selectRecentActivity(@Param("userId") Long userId, @Param("limit") int limit);

    /** 活跃日期列表（连续天数计算用） */
    List<LocalDate> selectActiveDates(@Param("userId") Long userId);

    /** 学习记录星期分布 */
    List<Map<String, Object>> selectStudyWeekdayDistribution(@Param("userId") Long userId);
